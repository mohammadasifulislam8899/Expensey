...
interface BudgetDao {
    ...
    @Query("SELECT * FROM budgets WHERE syncStatus != 'SYNCED' AND isDeleted = 0")
    ...
    @Query("UPDATE budgets SET isDeleted = 1, syncStatus = 'PENDING' WHERE id = :id")
    suspend fun markBudgetDeleted(id: String)

    @Query("UPDATE budgets SET syncStatus = 'SYNCED' WHERE id = :id")
    suspend fun markBudgetSynced(id: String)
    ...
    @Query("DELETE FROM budgets WHERE syncStatus = 'SYNCED' AND isDeleted = 0")
